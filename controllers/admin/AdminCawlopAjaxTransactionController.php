...
class AdminCawlopAjaxTransactionController extends ModuleAdminController
{
    ...
    public function displayAjaxCapture()
    {
        ...
        try {
            ...
            $this->module->getLogger()->error('Capture request', ['request' => json_decode($capturePaymentRequest->toJson(), true)]);
            $this->module->getLogger()->error('Response exception', ['response' => json_decode($re->getResponse()->toJson(), true)]);
        } catch (Exception $e) {
    ...
    public function displayAjaxRefund()
    {
        ...
        try {
            ...
            $this->context->smarty->assign('worldlineopAjaxTransactionError', $re->getMessage());
            $this->module->getLogger()->error('Refund request', ['request' => json_decode($refundRequest->toJson(), true)]);
            $this->module->getLogger()->error('Response exception', ['response' => json_decode($re->getResponse()->toJson(), true)]);
        } catch (Exception $e) {
            $this->context->smarty->assign('worldlineopAjaxTransactionError', $e->getMessage());
            $this->module->getLogger()->error($e->getMessage());
    ...
    public function displayAjaxCancel()
    {
        ...
        if (isset($cancelResponse)) {
            $this->module->getLogger()->debug('Cancel response', ['json' => json_decode($cancelResponse->toJson(), true)]);
